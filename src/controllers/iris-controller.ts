...
const querySchema = z.object({
  ...
  skip: z.number().min(0),
  sort: z.enum(['sepalLength', 'sepalWidth', 'petalLength', 'petalWidth', 'species']),
...
export const getAllIrisData = async (req: Request, res: Response) => {
  try {
    ...
    if (validatedQuery.limit && validatedQuery.skip) {
      return res.json(await IrisModel.find().limit(validatedQuery.limit).skip(validatedQuery.skip));
      ...
      return res.json(await IrisModel.find().sort({[validatedQuery.sort]: validatedQuery.order}));
    } else if (validatedQuery.sort) {
      return res.json(await IrisModel.find().sort({[validatedQuery.sort]: 'asc'}));
    } else {
...
export const updateIrisData = async (req: Request, res: Response) => {
  try {
    return res.json(await IrisModel.findByIdAndUpdate(idSchema.parse(req.params)._id, irisSchema.parse(req.body), {new: true}));
  } catch (error) {
    console.error(error);
    return res.status(500).json(error);
  }
...
export const deleteIrisData = async (req: Request, res: Response) => {
  try {
    return res.json(await IrisModel.findByIdAndDelete(idSchema.parse(req.params)._id));
  } catch (error) {
    console.error(error);
    return res.status(500).json(error);
  }
